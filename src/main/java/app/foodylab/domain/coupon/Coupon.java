...
public class Coupon {
    ...
    public static Coupon of(final Long discountAmount, String nameOfCoupon,
        CouponType typeOfCoupon) {
        require(discountAmount > 0, "할인 금액은 0보다 커야 합니다.");
        LocalDate now = LocalDate.now();
        return of(discountAmount, nameOfCoupon, typeOfCoupon, now, now.plusYears(1));
    }

    public static Coupon of(final long discountAmount, String nameOfCoupon, CouponType typeOfCoupon,
        final LocalDate startDate, final LocalDate endDate) {
        LocalDate now = LocalDate.now();
        require(discountAmount > 0, "할인 금액은 0보다 커야 합니다.");
        require(!startDate.isBefore(now), "쿠폰 사용 시작일은 현재보다 이전일 수 없습니다.");
        require(typeOfCoupon != null, "쿠폰 종류는 필수값 입니다.");
